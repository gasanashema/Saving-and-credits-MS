...
import { BellIcon, InformationCircleIcon, EnvelopeOpenIcon, PlusIcon, UserGroupIcon, PaperAirplaneIcon, XMarkIcon, ChatBubbleLeftRightIcon } from '@heroicons/react/24/outline';
...
const Notifications: React.FC = () => {
  ...
  const fetchAdmins = async () => {
    try {
      ...
      setAdmins(res.data.filter((u: any) => u.role === 'admin' || u.role === 'sadmin' || u.role === 'supperadmin'));
    } catch (error) {
  ...
  const markAsRead = async (id: number) => {
    try {
      ...
      // Trigger a refresh of the unread count in navbar
