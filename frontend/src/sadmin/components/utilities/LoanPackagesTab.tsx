...
const LoanPackagesTab: React.FC = () => {
  const { packages, loading, error, refresh } = useLoanPackages();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingPackage, setEditingPackage] = useState<LoanPackage | null>(
    null,
  );
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400">
            Manage loan configurations and eligibility rules.
          </p>
        </div>
        <button
          onClick={() => {
            ...
            <div className="p-5">
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <span
                    className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium mt-2 ${pkg.status === "active" ? "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300" : "bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300"}`}
                  >
                    {pkg.status}
                  </span>
                </div>
              ...
              <div className="space-y-2 text-sm text-gray-600 dark:text-gray-300">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">
                    {(pkg.min_savings || 0).toLocaleString()} RWF
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Max Loan Amount:</span>
                  <span className="font-semibold text-green-600 dark:text-green-400">
                    {(pkg.max_loan_amount || 0).toLocaleString()} RWF
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Min Membership:</span>
                  <span className="font-semibold">
                    {pkg.min_membership_months} months
                  </span>
                </div>
                ...
                <div className="flex justify-between">
                  <span>Duration:</span>
                  <span className="font-semibold">
                    {pkg.repayment_duration_months} months
                  </span>
                </div>
              </div>
            </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        repayment_duration_months: Number(e.target.value),
                      })
                    }
                    className="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500"
                  />
...
export default LoanPackagesTab;
