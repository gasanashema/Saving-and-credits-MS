...
import { BanknotesIcon, CreditCardIcon, ClockIcon, CheckCircleIcon, XCircleIcon, ArrowLeftIcon } from '@heroicons/react/24/outline';
import { toast } from 'sonner';

// Reusing parts of the modal logic from Loans.tsx but adapted for a page
const MemberLoanDetail: React.FC = () => {
    ...
    useEffect(() => {
        const fetchLoan = async () => {
            try {
                setLoading(true);
                // Assuming there's an endpoint to get a single loan or we filter from list? 
                // Best to maintain consistency with existing API patterns. 
                // If /loans/:id doesn't exist, we might need to implement it or use existing list endpoint.
                // Let's assume we can fetch all and find, or if the user added a specific endpoint.
                // Checking existing hooks... useMemberLoans fetches all. 
                // We probably need a specific endpoint or just fetch all and find for now.
                // Actually, let's try to fetch all and find, since I didn't see a "get single loan" endpoint in previous files.
                // However, a scalable app should have it. I'll rely on fetching specific loan if I add that endpoint or just use what we have.

                // I'll add a specific endpoint to backend if needed, but for now let's try to simulate fetching.
                // In useMemberLoans (hooks), it fetches /loans/member/:memberId.
                // We assume we can fetch the specific loan from the new endpoint.

                const response = await server.get(`/loans/details/${id}`); // Validation needed if this endpoint exists
                ...
                try {
                    ...
                    console.log("No payment details found or error", e);
                }
    ...
    // Normalization (similar to Loans.tsx)
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md overflow-hidden">
                <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-gray-500">{new Date(loan.requestDate).toLocaleDateString()}</p>
                    </div>
                ...
                <div className="p-6 space-y-6">
                    ...
                    {/* Payment History from paymentDetails */}
