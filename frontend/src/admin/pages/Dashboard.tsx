...
const fetchAdminDashboardData = async () => {
  try {
    const [membersRes, savingsRes, loansRes, paymentsRes, penaltiesRes] =
      await Promise.all([
        server.get("/members"),
...
const processAdminData = (rawData: any) => {
  ...
  const totalSavings = savings.reduce(
    (sum: number, s: any) => sum + Number(s.total || s.amount || 0),
    0,
  );
  const activeLoansAmount = loans
    .filter((l: any) => l.lstatus === "active" || l.status === "active")
  ...
  const profits = loans
    ...
    .reduce(
      (sum: number, l: any) =>
        sum + (Number(l.payedAmount || 0) - Number(l.amount || 0)),
      0,
    );

  const unpaidPenalties = penalties
    .filter((p: any) => p.pstatus !== "paid")
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <motion.div variants={item}>
          <StatsCard
            ...
            value={formatCompactCurrency(data.stats.totalSavings)}
        ...
        <motion.div variants={item}>
          <StatsCard
            title={t("activeLoans")}
            value={formatCompactCurrency(data.stats.activeLoansAmount)}
        ...
        <motion.div variants={item}>
          <StatsCard
            title="Profits"
            value={formatCompactCurrency(data.stats.profits)}
            icon={<BanknotesIcon className="h-6 w-6" />}
            bgColor="bg-white dark:bg-gray-800"
            textColor="text-gray-800 dark:text-white"
            iconBgColor="bg-blue-500"
