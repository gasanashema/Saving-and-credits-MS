...
const sendMessage = async (req, res) => {
  try {
    ...
    if (!['admin', 'member'].includes(receiverType)) {
      return res.status(400).json({ error: 'receiverType must be "admin" or "member"' });
    }
    ...
    // Insert chat
    console.debug('chat.send: payload', { senderType, senderId, receiverType, receiverId: toReceiverId, message: message?.slice?.(0,200) });
    ...
    const [rows] = await conn.query('SELECT id, sender_type, sender_id, receiver_type, receiver_id, message, is_read, created_at FROM chats WHERE id = ?', [insertedId]);
    ...
    try {
      if ((senderType === 'admin') && (receiverType === 'member')) {
        ...
        await createNotification({ senderAdminId: senderId, receiverType: 'member', receiverId, title, message: shortMsg, url });
      }
    } catch (e) {
...
const getConversation = async (req, res) => {
  try {
    ...
    // Validate otherType
    if (!['admin','member'].includes(otherType)) return res.status(400).json({ error: 'Invalid otherType' });

    // Members may only fetch conversations with admins; admins can fetch any conversation
    if (decoded.role === 'member' && otherType !== 'admin') return res.status(403).json({ error: 'Members can only view conversations with admins' });
...
const getPartners = async (req, res) => {
  try {
    ...
    if (myType === 'member') {
      // list admins
      const [admins] = await conn.query("SELECT user_id, fullname, email FROM users WHERE role IN ('admin','supperadmin','sadmin')");
      const result = await Promise.all(admins.map(async (admin) => {
        const [last] = await conn.query('SELECT id, sender_type, sender_id, receiver_type, receiver_id, message, is_read, created_at FROM chats WHERE (sender_type = ? AND sender_id = ? AND receiver_type = ? AND receiver_id = ?) OR (sender_type = ? AND sender_id = ? AND receiver_type = ? AND receiver_id = ?) ORDER BY created_at DESC LIMIT 1', ['admin', admin.user_id, 'member', myId, 'member', myId, 'admin', admin.user_id]);
        const [unread] = await conn.query('SELECT COUNT(*) as c FROM chats WHERE sender_type = ? AND sender_id = ? AND receiver_type = ? AND receiver_id = ? AND is_read = 0', ['admin', admin.user_id, 'member', myId]);
        return { partnerType: 'admin', user_id: admin.user_id, fullname: admin.fullname, email: admin.email, lastMessage: last[0] || null, unreadCount: unread[0].c };
      }));
      return res.json(result);
    } else {
      // admin: list members who have chatted with this admin
      const [members] = await conn.query('SELECT DISTINCT m.id, m.firstName, m.lastName, m.telephone FROM chats c JOIN members m ON ((c.sender_type = \"member\" AND c.sender_id = m.id) OR (c.receiver_type = \"member\" AND c.receiver_id = m.id)) WHERE (c.sender_type = ? AND c.sender_id = ?) OR (c.receiver_type = ? AND c.receiver_id = ?) ORDER BY m.firstName', ['admin', myId, 'admin', myId]);
      const result = await Promise.all(members.map(async (member) => {
        const [last] = await conn.query('SELECT id, sender_type, sender_id, receiver_type, receiver_id, message, is_read, created_at FROM chats WHERE (sender_type = ? AND sender_id = ? AND receiver_type = ? AND receiver_id = ?) OR (sender_type = ? AND sender_id = ? AND receiver_type = ? AND receiver_id = ?) ORDER BY created_at DESC LIMIT 1', ['admin', myId, 'member', member.id, 'member', member.id, 'admin', myId]);
        const [unread] = await conn.query('SELECT COUNT(*) as c FROM chats WHERE sender_type = ? AND sender_id = ? AND receiver_type = ? AND receiver_id = ? AND is_read = 0', ['member', member.id, 'admin', myId]);
        return { partnerType: 'member', id: member.id, fullname: `${member.firstName} ${member.lastName}`, telephone: member.telephone, lastMessage: last[0] || null, unreadCount: unread[0].c };
      }));
      return res.json(result);
    }
  } catch (error) {
