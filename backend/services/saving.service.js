...
const addSavig = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Invalid numberOfShares or shareValue" });
    }
...
const getAllSavings = async (req, res) => {
  ...
  try {
    ...
    let query =
      "SELECT members.id AS id, `sav_id`, `nid`, `firstName`, `lastName`, telephone, savings.date, savings.shareValue, `numberOfShares`, (savings.shareValue * savings.numberOfShares) as total FROM `members` INNER JOIN savings ON members.id = savings.memberId";
...
const createSavingType = async (req, res) => {
  try {
    ...
    const [result] = await conn.query(
      "INSERT INTO savingtypes (title, Description, amount) VALUES (?, ?, ?)",
      [title, description || "", amount],
    );
    return res
      .status(201)
      .json({ message: "Saving type created", id: result.insertId });
  } catch (error) {
...
const updateSavingType = async (req, res) => {
  try {
    ...
    const { title, description, amount } = req.body;
    await conn.query(
      "UPDATE savingtypes SET title=?, Description=?, amount=? WHERE stId=?",
      [title, description || "", amount, id],
    );
    return res.json({ message: "Saving type updated" });
  } catch (error) {
    return res.status(500).json({ error: error.message });
  }
};
...
const deleteSavingType = async (req, res) => {
  try {
    const { id } = req.params;
    await conn.query("DELETE FROM savingtypes WHERE stId=?", [id]);
    return res.json({ message: "Saving type deleted" });
  } catch (error) {
    return res.status(500).json({ error: error.message });
  }
};
